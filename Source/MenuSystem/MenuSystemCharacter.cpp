...
AMenuSystemCharacter::AMenuSystemCharacter() :
	CreateSessionCompleteDelegate(FOnCreateSessionCompleteDelegate::CreateUObject(this, &ThisClass::OnCreateSessionComplete)), // Initialize delegate 
...
void AMenuSystemCharacter::CreateGameSession()
{
	...
	// Check if one already exists. Destroy it if it does so we can create a new session. NAME_GameSession is a global variable
	...
	// Adding our own delegate to the create session delegate list upon completion
	OnlineSessionInterface->AddOnCreateSessionCompleteDelegate_Handle(CreateSessionCompleteDelegate);
	
	// Actually creating the session with our session settings
...
void AMenuSystemCharacter::JoinGameSession()
{
	...
	SessionSearch->MaxSearchResults = 10000; // High number as we're using Steam ID 480 which is used by lots of people
...
void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		...
		// Travel to the Lobby level and open as listen server
